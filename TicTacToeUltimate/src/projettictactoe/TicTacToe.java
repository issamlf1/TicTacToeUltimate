...
public class TicTacToe extends GameSearch {
    ...
    //test si toutes les cases sont plein true else retourn false
    public boolean drawnPosition(Position p) {
        ...
        return ret;
    }//verification si un player à gagner
    ...
    public Position [] possibleMoves(Position p, boolean player) {
        ...
        Position [] ret = new Position[count];//tableau de position
        count = 0;
        for (int i=0; i<9; i++) {
            if (pos.board[i] == 0) {
    ...
    //?????????????????????????????????????????????????????????????????????????????????????????????????
    public boolean reachedMaxDepth(Position p, int depth, int maxDepth) {
        boolean ret = false;
        if(depth>=maxDepth) ret = true;//le niveau de difficulter augmente avec depth c'est la profondeur
        else 
        if (wonPosition(p, false)) ret = true;//si la machine a gagner
        else if (wonPosition(p, true))  ret = true;//si l'humain a gagner
        else if (drawnPosition(p))      ret = true;//  s'il ne reste pas de case null;
        if (GameSearch.DEBUG) {
    ...
    //elle lit la position entrée par le player
    public Move createMove() {
        if (GameSearch.DEBUG) System.out.println("Enter blank square index [0,8]:");
        int i = 0;
        try {
            int ch = System.in.read();
            i = ch - 48; //a une relation avec le code asqui
            //System.in.read();
            // System.in.read();
    ...
    static public void main(String [] args) {
        ...
        ttt.playGame(p, true,0);// si on mes faulse c'est la machine qui va commencer a jouer
    }
}
