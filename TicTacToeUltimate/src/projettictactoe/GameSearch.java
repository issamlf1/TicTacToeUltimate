...
public abstract class GameSearch {
    ...
    public abstract boolean reachedMaxDepth(Position p, int depth, int maxDepth);
    ...
    protected Vector alphaBeta(int depth, Position p, boolean player, int maxDepth) {
        Vector v = alphaBetaHelper(depth, p, player, 1000000.0f, -1000000.0f, maxDepth);// ?? il a une relatio avec le min et le max on inversse
        //System.out.println("^^ v(0): " + v.elementAt(0) + ", v(1): " + v.elementAt(1));
        return v;
    }

    protected Vector alphaBetaHelper(int depth, Position p,
                                     boolean player, float alpha, float beta, int maxDepth) {
        if (GameSearch.DEBUG) System.out.println("alphaBetaHelper("+depth+","+p+","+alpha+","+beta+")");
        if (reachedMaxDepth(p, depth, maxDepth)) { //????????????????????????,
        ...
        Position [] moves = possibleMoves(p, player);//creation de l'arbre
        for (int i=0; i<moves.length; i++) {
            Vector v2 = alphaBetaHelper(depth + 1, moves[i], !player, -beta, -alpha,maxDepth);
    ...
    public Position playGame(Position startingPosition, boolean humanPlayFirst, int maxDepth) {
...
//        }
       // while (true) {
            //printPosition(startingPosition);//affiche la grille
...
//            }
//            System.out.print("\nYour move:");
//            Move move = createMove();//choix de la position par l'utilisateur et le renvoi de la Move
//            startingPosition = makeMove(startingPosition, HUMAN, move);
//           printPosition(startingPosition);
            Vector v = alphaBeta(0, startingPosition, PROGRAM,maxDepth);
//            System.err.println(v);
//            System.err.println("***********");
...
//            try {
//                    System.in.read();//pour donner la possibilite a ecrire
//            } catch (Exception e) { }
        //}
    }
}
